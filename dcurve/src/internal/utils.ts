 ...
 */
export const addAnonContribution = (grantId: number, grantRoundContributions: GrantRoundContributions, amount: number): GrantRoundContributions  => {
...
export const getGrantMatch = (grantId: number, grantsDistribution: GrantsDistribution): number => {
  const contributions: GrantMatch[] = grantsDistribution.distribution.filter(grantMatch => 
    grantMatch.grantId=grantId
  );
